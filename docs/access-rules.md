...
- `general_director` — глобальная видимость workspace, но не права CEO на projects, tasks и Settings.
- `none` — глобальных привилегий нет.

Флаги Django `is_staff` и `is_superuser` дают глобальную видимость workspace, но сами по себе не дают права CEO на projects, tasks и Settings.

### Участие в компании (`CompanyMembership.role`)

- `director` — видит workspace своей компании и может управлять projects в нем.
...
Пользователь видит team, если он имеет глобальную видимость либо подходит под один из grants, входящих в team: user, company или department.

Системные department teams автоматически формируются по `DepartmentType`. Страницы списка, деталей и состава доступны только для чтения. Публичные POST-запросы создания, редактирования и изменения состава возвращают `405`; соответствующих форм в рабочем UI нет.

Team состоит из department-level `WorkspaceAccessGrant`; само участие в team не создает доступ к workspace. Менять teams может только внутренний synchronization service.
...
Оба правила включают создание project и редактирование его основных данных. Назначить или сменить автоматически сформированную team через `Project Members` может только `ceo`; остальные видят привязанную team и departments read-only.
...
| Изменить team или ее состав | Нет | Нет | Нет | Нет | Нет |
| Сменить team проекта | Да | Нет | Нет | Нет | Нет |
